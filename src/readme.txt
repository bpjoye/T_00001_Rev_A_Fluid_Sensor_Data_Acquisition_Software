 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software (application created by MATLAB Compiler)
		* run_T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.sh (shell script that can be run 
	...
	All Platforms:            
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.dll (shared library, file extension 
		  varies by platform)
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.h (library header file)
		* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.ctf (Component Technology File archive,
	...
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.xml (documentation files)
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.pdb (program database file - if 
	  DEBUG option selected)
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.jar (Java class files and compiled M-code)
	...
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software.xla (Optionally created and placed in
	  C:\Users\Watertech\Desktop\T_00001_Rev_A_Fluid_Sensor_Data_Acquisition_Software\src directory by the user)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
